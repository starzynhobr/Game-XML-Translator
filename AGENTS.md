...
- Use concise English for progress/status messages.
- Do not narrate obvious tool actions.
- Do not restate the task unless needed for clarification.
- Prefer direct implementation over long explanations.
- Report only important findings, decisions, blockers, test results, and final changes.
- Keep code comments and user-facing text consistent with the project's existing language/style.
